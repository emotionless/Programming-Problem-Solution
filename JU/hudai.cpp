...
//    cout<<"sdf";
    ...
    rep(i,1,n)
    {
        ...
        ans += min( a[ lft[idx] ] , a[ rgt[idx] ] );

        if( a[ lft[idx] ] > a[ lft[ rgt[idx] ] ] )
        {
            lft[ rgt[ idx ] ] = lft[ idx ];
        }

        if( a[rgt[ idx ]] > a[ rgt[ lft[idx] ] ] )
        {
            rgt[ lft[idx] ] = rgt[idx];
        }

        cout<<"id "<<idx<<" "<<lft[idx]<<" "<<rgt[idx]<<" "<<min( a[ lft[idx] ] , a[ rgt[idx] ] )<<endl;
    }
