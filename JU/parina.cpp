...
template<class T,class X> int getbit(T s,X i)
{
    return (s >> i) & 1;
}
...
ll p[MX] , fact[MX] , n , tree[MX] , res[MX];
...
void calc()
{
    ...
    rep(i,3,lmt)
    {
        res[i] = (((res[i-1]*i)%MOD)  +  (fact[i-1]*( ((ll)i*(ll)(i-1)/2)%MOD ))%MOD)%MOD;
    }
}
...
//
    ...
    rep(i,1,n)
    {
        ...
        ll sm  = sum(p[i]);
        ll lft = p[i] - 1 - sm;
        ans  = ( ans  + (ll)((((lft*(lft-1)/2)%MOD)*fact[n-i])%MOD)

                 + (ll)((tot*((lft*fact[n-i])%MOD))%MOD)

                 +  (ll)( (lft*res[n-i])%MOD)  ) %MOD;
