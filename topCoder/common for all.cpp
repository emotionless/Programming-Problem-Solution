...
int get(string first, string second)
{
    for(int l=min(first.length(), second.length()); l>=0; l--)
    {
        if(first.substr(0,l)==second.substr(second.length()-l))
            return l;
    }
    return 0;
}
...
string solve(string text, string pre, string suf)
{
    int len=text.length();
    int a=-1, b=-1;
    string ret="";
    ...
    for(int i=0; i<len; i++)
    {
        for(int j=1; j<=len-i; j++)
        {
            ...
            int first=get(str, pre);
            int second=get(suf, str);
...
int main()
{
    ...
    cin>>str1>>str2>>str3;
    string get=solve(str1, str2, str3);
    cout<<get<<endl;
