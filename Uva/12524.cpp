...
{
    return cost[j] - cost[i] - cs[i-1]*(arr[j].xx - arr[i].xx);
}
...
//            cout<<k<<"        "<<jmid<<"          "<<ret<<" "<<costFunc(k, jmid)<<endl;
            bestK = k;
        }
    }
...
//freopen("inp.txt", "r", stdin);
    ...
    {
        for(i = 1; i <= n; i++) sc(arr[i].xx, arr[i].yy);
        ...
        for(i = 1; i <= n; i++)
        {
            cost[i] = cost[i-1] + cs[i-1]*(arr[i].xx - arr[i-1].xx);
            cs[i] = cs[i-1] + arr[i].yy;
//        cout<<i<<" "<<cost[i]<<" "<<cs[i]<<endl;
        }
