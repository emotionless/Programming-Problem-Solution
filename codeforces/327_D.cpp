...
int BFS(int x, int y)
{
    ...
    while(!Q.empty())
    {
        ...
        vis[cx][cy] = 1;
        if(ch[cx][cy] != '.' && ch[cx][cy] != '#' && ch[x][y] != ch[cx][cy])
        {
            return abs(cx - x) + abs(cy - x);
        }
        for(int i = 0; i < 4; i++)
        {
            int adx = cx + dr[i];
...
//     fp=fopen("out.txt", "w");
    ...
    for(i = 0; i < n; i++)
    {
        for(j = 0; j < m; j++)
        {
            if(ch[i - 1][j] == ch[i][j] && ch[i + 1][j] == ch[i][j] && ch[i][j - 1] == ch[i][j] && ch[i][j + 1] == ch[i][j]) continue;
            BFS(i, j);
        }
    }
