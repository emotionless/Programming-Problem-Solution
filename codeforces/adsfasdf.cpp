...
int MIN(int k, int i, int val)
{
    int st = 0, ed = V[k][i].size()-1, ans = V[k][i].size(), mid = 0;
...
int MXX(int k, int i, int val)
{
    int st = 0, ed = V[k][i].size()-1, ans = -1, mid = 0;
...
//        V[s][x].pb(y);
    ...
    for(k = 0; k <= 10; k++)
    {
        for(i= 1; i <= 100; i++)
        {
            for(j = 1; j <= 100; j++)
            {
                dp[k][i][j] = dp[k][i-1][j] + dp[k][i][j-1] - dp[k][i-1][j-1] + val[k][i][j];
            }
        }
    }
    int t;
    ...
    while(q--)
    {
        ...
        for(k = 0; k <= 10; k++)
        {
            ans += ((t+k)%(c+1)) * (   dp[k][edx][edy]- dp[k][edx][sty-1] - dp[k][stx-1][edy] + dp[k][stx-1][sty-1]  );
        }
        printf("%d\n", ans);
