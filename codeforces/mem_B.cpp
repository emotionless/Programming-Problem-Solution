...
void binary_search(LL st, LL ed, LL tar)
{
    LL mid=(st+ed)/2;
    ...
    while(st<ed)
    {
        ...
        LL tmp=(n-(mid*(mid-1))/2)/mid;
        if(tmp==tar)
        {
            if((tar*mid+(mid*(mid-1))/2)==n)
            {
                V.PB((tar+1)*mid);
                return;
            }
...
int main()
{
    ...
    FOR(i,0,63)
    {
        binary_search(1,MAX,power2[i]-1);
    }
