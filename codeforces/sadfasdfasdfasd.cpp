...
ll solve(int ind, int taken, bool is)
{
    if(ind == len)
    {
        if(step[taken] == k && is) return 1ll;
        return 0ll;
    }
//    if(step[taken] == k) return 1ll;
    ll &ret = dp[ind][taken][is];
    if(ret != -1ll) return ret;
    ret = 0ll;
    int l = is?1:(str[ind]-'0');
    for(int i = 0; i <= l; i++)
        ret = (ret + solve(ind + 1, taken + (i==1?1:0), is|(i<l)) )%MOD;
return ret;
}
